...
/**
 * ALL FOLLOWING CODE ADDED BY RAJ SHAH NOV 7 2012
 **/
...
function custom_override_checkout_fields( $fields ) {
	...
	$fields['billing']['billing_state']['class'] = 'form-row form-row-first update_totals_on_change';
...
function my_custom_checkout_field( $checkout ) {
	...
	woocommerce_form_field( 'childs_name', array(
		'type'			=> 'text',
		'class'			=> array('my-field-class form-row-first'),
		'label'			=> __('Child\'s Name'),
		'placeholder'	=> __('First and Last Name'),
		'required'		=> true,
		), $checkout->get_value('childs_name'));
	
	woocommerce_form_field( 'childs_grade', array(
		'type'			=> 'text',
		'class'			=> array('my-field-class form-row-first'),
		'label'			=> __('Child\'s Current Grade'),
		'placeholder'	=> __('e.g. preK, KG, 1st, 2nd, ...'),
		'required'		=> true,
		), $checkout->get_value('childs_grade'));
...
function my_custom_checkout_field_process() {
	...
	if (!$_POST['childs_name'])
		$woocommerce->add_error(__('Please Enter Your Child\'s Full Name prior to checkout.') );

	// Check if set, if its not set add an error.
	if (!$_POST['childs_grade'])
		$woocommerce->add_error(__('Please Enter Your Child\'s grade level prior to checkout.') );
}
...
function my_custom_checkout_field_update_order_meta( $order_id ) {
	if ($_POST['childs_name']) update_post_meta( $order_id, 'Child Name', esc_attr($_POST['childs_name']));
	if ($_POST['childs_grade']) update_post_meta( $order_id, 'Child Grade', esc_attr($_POST['childs_grade']));
